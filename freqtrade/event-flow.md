...
flowchart TD
    ...
    subgraph "Strategy Initialization"
...
flowchart TD
    ...
    subgraph "For Each Trade"
        ...
        CheckStoploss -->|Triggered| PrepareExit[Prepare Exit]
        CheckROI -->|Triggered| PrepareExit
        CheckExitSignal -->|Triggered| PrepareExit
        CheckCustomExit -->|Triggered| PrepareExit
        ...
        ConfirmExit -->|Confirmed| ExecuteExit[Execute Exit Order]
        
        LoopTrades --> CheckAdjustment[Call adjust_trade_position()]
        CheckAdjustment -->|Adjustment Needed| ExecuteAdjustment[Execute Adjustment Order]
    end
```
...
flowchart TD
    ...
    CheckSlots -->|Slots Available| LoopPairs[Loop Through Pairs]
    
    subgraph "For Each Pair with Entry Signal"
        ...
        ConfirmEntry -->|Confirmed| ExecuteEntry[Execute Entry Order]
    end
```
