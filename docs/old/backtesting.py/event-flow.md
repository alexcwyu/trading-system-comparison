...
flowchart TD
    ...
    InitStrategy --> CalculateIndicators
```
...
flowchart TD
    ...
    CallNext --> CheckSignals[Check signals/conditions]
    CheckSignals --> GenerateOrders[Generate orders]
    GenerateOrders --> ManageTrades[Manage existing trades]
    ...
    ProcessPendingOrders --> ProcessMarketOrders[Process market orders]
    ProcessMarketOrders --> CheckLimitOrders[Check limit orders]
    CheckLimitOrders --> CheckStopOrders[Check stop orders]
    CheckStopOrders --> UpdatePositions[Update positions]
```
