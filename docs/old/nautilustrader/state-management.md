...
graph TD
    ...
    class Database,Checkpoints,EventJournal,Redis persist;
```
...
The `Cache` is the central state repository that holds all trading-related state with O(1) constant-time lookups. It can be configured to use either an in-memory implementation or a Redis-backed implementation for distributed deployments.
...
The Rust implementation of the `Cache` provides several key advantages:
...
6. **Persistence**: State can be persisted to disk, database, or Redis
...
9. **Memory Safety**: Rust's ownership model ensures memory safety and prevents data races

### Value Types and Precision Modes

NautilusTrader supports two precision modes for its core value types, affecting how state is stored and manipulated:
...
classDiagram
    ...
    PrecisionMode --> Price: defines
    PrecisionMode --> Quantity: defines
    PrecisionMode --> Money: defines
```

#### Precision Modes

- **Standard Precision Mode**
  - Uses 64-bit integers internally
  - Supports up to 9 decimal places
  - Faster operations, especially on platforms without native 128-bit support (e.g., Windows)
  - Suitable for most trading applications

- **High Precision Mode**
  - Uses 128-bit integers internally
  - Supports up to 16 decimal places
  - Provides greater range and precision for specialized applications
  - Default on Linux and macOS platforms
  - Perfect for HFT where price precision is critical

The precision mode affects the internal representation of all value types, including prices, quantities, and monetary values. This unified approach ensures consistent precision throughout the system.
