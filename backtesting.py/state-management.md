...
```mermaid
stateDiagram-v2
    [*] --> Initialization: Create Backtest
    Initialization --> BarProcessing: Run Backtest
    
    state Initialization {
        [*] --> StrategyInit: Create Strategy
        StrategyInit --> IndicatorCalc: Calculate Indicators
        IndicatorCalc --> [*]: Ready for simulation
    }
    
    state BarProcessing {
        [*] --> ProcessOrders: For each bar
        ProcessOrders --> UpdateData: Update available data
        UpdateData --> CallNext: Call Strategy.next()
        CallNext --> CheckSignals: Check trading signals
        CheckSignals --> PlaceOrders: Place new orders
        PlaceOrders --> UpdatePosition: Update position
        UpdatePosition --> UpdateEquity: Update equity
        UpdateEquity --> [*]: Move to next bar
    }
    
    BarProcessing --> Finalization: All bars processed
    Finalization --> [*]: Return results
    
    state "Order Lifecycle" as OrderState {
        [*] --> Pending: Order created
        Pending --> Active: Order activated
        Active --> Filled: Price conditions met
        Active --> Canceled: Order canceled
        Filled --> [*]: Trade created
        Canceled --> [*]
    }
    
    state "Trade Lifecycle" as TradeState {
        [*] --> Open: Trade opened
        Open --> Monitoring: Monitor trade
        Monitoring --> CheckSL: Check stop-loss
        Monitoring --> CheckTP: Check take-profit
        Monitoring --> ManualClose: Manual close
        CheckSL --> Closed: SL triggered
        CheckTP --> Closed: TP triggered
        ManualClose --> Closed: Manually closed
        Closed --> [*]: Update position
    }
```
